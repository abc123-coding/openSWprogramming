...
int main() {
	...
	bool NonMaxSupp = false; //Option for the non-maximum suppression
	bool Subpixel = true; 	//Option for subpixel refinement in corner detection,

	cvtColor(input, input_gray, CV_RGB2GRAY);	// convert RGB to Grayscale	

	//Harris corner detection using 'cornerHarris'
	cornerHarris(input_gray, output, 2, 3, 0.03);
	...
	if (NonMaxSupp)
	{
		...
		NonMaximum_Suppression(input_gray, corner_mat, radius);
	...
	if (Subpixel)
	{
		...
		// Set the sub-pixel refinement parameters
		Size subPixWinSize(3, 3);
		TermCriteria termcrit(TermCriteria::COUNT | TermCriteria::EPS, 20, 0.03);

		// Refine the corner locations using sub-pixel accuracy
		cornerSubPix(input_gray, points, subPixWinSize, Size(-1, -1), termcrit);

		// Display the set of refined corners
		input_visual = input.clone();
		for (int k = 0; k < points.size(); k++) {
			...
			circle(input_visual, Point(x, y), 2, Scalar(0, 255, 0), 1, 8, 0);
		}
...
Mat NonMaximum_Suppression(const Mat input, Mat corner_mat, int radius)
{
	...
	for (int i = radius; i < row + radius; i++) {
		for (int j = radius; j < col + radius; j++) {
			...
			if (corner_mat.at<float>(i - radius, j - radius) > 0) {

				bool is_maximum = true;

				// iterate over the neighboring pixels in the window
				for (int k = i - radius; k <= i + radius; k++) {
					for (int l = j - radius; l <= j + radius; l++) {

						// check if the neighboring pixel is greater than the current pixel
						if (input_mirror.at<float>(k, l) > input_mirror.at<float>(i, j)) {
							is_maximum = false;
							break;
						}
					}
				...
				// if the current pixel is not the maximum in the window, set its value to zero
				if (!is_maximum) {
					corner_mat.at<float>(i - radius, j - radius) = 0;
				}
			}
		}
	}
